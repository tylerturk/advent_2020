...
def adjacents_with_energy(pd, start_x, start_y, start_z, start_w):
    """
    ...
    for x in range(start_x-1, start_x+2):
        for y in range(start_y-1, start_y+2):
            for z in range(start_z-1, start_z+2):
                for w in range(start_w-1, start_w+2):
                    if x == start_x and y == start_y and z == start_z and w == start_w:
                        continue
                    try:
                        if pd[x][y][z][w] == '#':
                            adjacent_energy += 1
...
def evaluate_pocket_dimension(pd):
    ...
    w_keys = pd[0][0][0].keys()
    low_w_bound = int(min(w_keys)) - 1
    high_w_bound = int(max(w_keys)) + 1
    for x in range(low_x_bound, high_x_bound + 1):
        for y in range(low_y_bound, high_y_bound + 1):
            for z in range(low_z_bound, high_z_bound + 1):
                for w in range(low_w_bound, high_w_bound + 1):
                    if x not in new_pd:
                        new_pd[x] = {}
                    if y not in new_pd[x]:
                        new_pd[x][y] = {}
                    if z not in new_pd[x][y]:
                        new_pd[x][y][z] = {}
                    active = False
                    try:
                        active = pd[x][y][z][w] == '#'
                        ...
                        pass
                    num_active = adjacents_with_energy(pd, x, y, z, w)
